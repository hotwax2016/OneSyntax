...
@section('title')
Edit city | System Management
@endsection

@section('main')
<div class="w-full">
	<h1 class="text-xl text-gray-600">Edit city</h1>
	<div class="w-2/3 mt-16 mx-auto bg-white p-6 border rounded-lg">
		<form method="POST" action="/cities/{{$city->id}}" class="space-y-4">
			@csrf
			@method('PUT')
			<div class="flex w-full">
				<div class="flex-col w-full pr-2">
					...
					<select name="country">
						<option value="{{ $city->name }}" selected disabled>{{ $city->get_state->name }}</option>
					</select>
				</div>
				<div class="flex-col w-full pl-2">
					<label class="block w-full pl-1 text-md text-gray-600" for="">Name</label>
					<input class="block w-full mt-1 p-2 text-gray-500 border rounded-lg shadow-sm" type="text" name="name" value="{{$city->name}}">
					@error('name')
